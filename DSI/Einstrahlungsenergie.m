...
% Gl.5.18 auf Seite 50.
function handles = Einstrahlungsenergie(handles)
%EINSTRAHLUNGSENERGIE Summary of this function goes here
%   Detailed explanation goes here
% Wetter Zustand
tag=str2num(handles.struct_sgd.solar_day);
ort_breite=str2num(handles.struct_sgd.ort_breite);
ort_lange=str2num(handles.struct_sgd.ort_lange);
%Globalstrahlung
G_on=1367*(1+0.033*cos(2*pi*tag/180));

wetter_jahr=wetterbedingung();

%Sonnenposition
J=360*tag/365;
delta=0.3948-23.2559*cos(pi*(J+9.1)/180)-0.3915*cos(pi*(2*J+5.4)/180)-...
    0.1764*cos(pi*(3*J+105.2)/180);
C=(0.0066+7.3525*cos(pi*(J+85.9)/180)+9.9359*cos(pi*(2*J+108.9)/180)+...
    0.3387*cos(pi*(3*J+105.2)/180))/60;

if tag > 89 && tag < 299
    MOZ=(-1:0.25:22.75)-4*(15-ort_lange)/60;
else
...
handles.solar_Gamma_s(tag, :)=asin(cos(pi*omega/180)*cos(pi*ort_breite/180)*cos(pi*delta/180)+sin(pi*ort_breite/180)*sin(pi*delta/180));
handles.solar_alpha_s(tag, MOZ<=12)=180-180/pi*acos((sin(handles.solar_Gamma_s(tag, MOZ<=12))*sin(pi*ort_breite/180)-...
    sin(pi*delta/180))./(cos(handles.solar_Gamma_s(tag, MOZ<=12))*cos(pi*ort_breite/180)));
handles.solar_alpha_s(tag, MOZ>12)=180+180/pi*acos((sin(handles.solar_Gamma_s(tag, MOZ>12))*sin(pi*ort_breite/180)-...
    sin(pi*delta/180))./(cos(handles.solar_Gamma_s(tag, MOZ>12))*cos(pi*ort_breite/180))); 
...
%Himmelklarheit
k_T=wetter_jahr(tag,:).*(1.1*(exp(-0.357)).^(AM.^0.678));
